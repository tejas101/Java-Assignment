...
public class RMIConsProdStorage extends Exception {
	...
	public static void main(String args[])
		...
		RMIConsProdStorage CP = new RMIConsProdStorage();
	 ...
	 */
	void Producer(int s, int prodLimit) throws
	InterruptedException, IOException, ClassNotFoundException {
		ProducerWork work = null;
		try {//Producer on Glados
			work = (ProducerWork)Naming.lookup("rmi://129.21.22.196:2002/WorkP");
			}catch (Exception e) {
				System.out.println("Storage logic exception: " + e);
				}
		while (true) {
				...
				if (tray.size() < s && plimit <= prodLimit) {
					...
					if (Collections.frequency(tray, 2) < 5) {
						for (int i = 0; i < 5; i++) {
							if (Collections.frequency(tray, 2) < 5 
									&& plimit < prodLimit) {
								tray=work.Pwork(tray,2);//Making a RMI call	
								plimit++;
...
IOException, ClassNotFoundException {
		ConsumerWork work = null;
		try {//Consumer of Queeg
			work = (ConsumerWork)Naming.lookup("rmi://129.21.30.37:2002/WorkC");
			}catch (Exception e) {
				System.out.println("Storage logic exception: " + e);
				}
		while (true) {
					...
					while (climit < 10) {
						for (int i = 0; i < 5; i++) {
							...
							if (element != -1) {
								...
								tray=work.Cwork(tray,element);//RMI call
							 //tray.remove(element);
								climit++;
								System.out.println(Thread.currentThread().getName() 
