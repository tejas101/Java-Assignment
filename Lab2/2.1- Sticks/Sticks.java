 ...
 *   
 *     
 *     Algorithm used : Dymamic Problem solving with boolean table.
 */
/** 
 * This program calculates the sum of subset from stickLengths
 * array by using least number of sticks for 
 * every number in unknowStickLengths array
 * 
 * @author      Tejas Raval 
 */
public class Sticks {
	static int[] stickLengths = { 1, 5, 8, 12, 12, 35, 35, 35, 61,25,25,50,105,50,204,205};
  ...
  */
   ...
   */
    ...
    {   
    	boolean [][] matrix = new boolean[stickLengths.length+1][check+1];
    	/**
    	  *Boolean matrix for Dynamic problem solving table. 
    	  */
    	for (int i = 0; i <= stickLengths.length; i++) {
    	 ...
    	 for(int i=1;i<=stickLengths.length; i++)
    		 for(int j=1; j<=check; j++)
    		 {
    			 ...
    			 /** 
                  *If the current stick length under processing is greater than the
                  *partial sum which we want, then just copy the value from the above cell
                  */
    			 }else {
    	 ...
    	 if (matrix[stickLengths.length][check]) {
    		 /** if this cell value is true, 
				 *then we have got a solution and now show the subset with addition 
				 *of least possible sticks to user
				 */
    		 System.out.print(+check+" inch: \tyes; used sticklenghts = ");
    	        for (int i=stickLengths.length; i>=1; i--) {  
    	            if (check!=0&&stickLengths[i-1]<=check){
    	            	/** If the value for the stick in stickLengths[i-1] is less 
    	            	 *than or equal to required sum, then include it only if the
    	            	 *reminder sum is possible to be achieved with the rest sticks
                         */
    	            	if(matrix[i-1][check-stickLengths[i-1]]) {
